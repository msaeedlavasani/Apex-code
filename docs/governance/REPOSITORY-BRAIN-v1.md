...
Adopt a lightweight repository-brain interface, but do not mechanically create seven duplicated brain files now. Before product implementation, create an Apex-native `AGENTS.md` as the behavior contract and add domain brain documents only when a domain has durable rules, clear ownership, and a validation surface.
