...
class DevelopmentControlPlaneTests(unittest.TestCase):
    ...
    def test_canonical_seed_preserves_event_ordering_task_and_adds_cli_probe(self):
        ...
        admission_task = next(item for item in backlog["tasks"] if item["task_id"] == "AC-DEV-018")
        self.assertEqual(admission_task["title"], "Execution Admission & Candidate Selection v1")
        self.assertEqual(admission_task["status"], "DONE")
        self.assertEqual(admission_task["verification_status"], "VERIFIED")
        self.assertEqual(admission_task["evidence_status"], "PROVEN")
        self.assertEqual(
        ...
        self.assertEqual(strategy["status"], "DONE")
        self.assertEqual(strategy["verification_status"], "VERIFIED")
        self.assertEqual(strategy["evidence_status"], "PARTIAL")
        self.assertEqual(strategy["human_gates"], [])
